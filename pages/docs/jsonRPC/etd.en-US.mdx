...
These methods are used for real-time events through subscriptions. See the <a href="https://Getd.ethereum.org/docs/rpc/pubsub" title="subscription documentation">subscription documentation</a> for more information.
...
| `gas`      | `Quality` | `<8`  | Yes      | Maximum gas allowance for the code execution to avoid infinite loops. Defaults to `2^63` or whatever value the node operator specified via `--rpc.gascap`.                                                                                                                                  |
| `gasprice` | `Quality` | `<32` | Yes      | Number of `wei` to simulate paying for each unit of gas during execution. Defaults to `1` `gwei`.                                                                                                                                                                                           |
| `value`    | `Quality` | `<32` | Yes      | Amount of `wei` to simulate sending along with the transaction. Defaults to `0`.                                                                                                                                                                                                            |
| `data`     | `Binary`  | any   | Yes      | Binary data to send to the target contract. Generally the 4 byte hash of the method signature followed by the ABI encoded parameters. For details please see <a href="https://github.com/ethereum/wiki/wiki/Ethereum-Contract-ABI" title="Ethereum Contract ABI">Ethereum Contract ABI</a>. |
...
With a synced Rinkeby node with RPC exposed on localhost (`Getd --rinkeby --http`) we can make a call against the Checkpoint Oracle to retrieve the list of administrators:
...
We can gut out the <a href="https://github.com/ethereum/go-ethereum/blob/master/contracts/checkpointoracle/contract/oracle.sol" title="original">original</a> checkpoint oracle contract with one that retains the same fields (to retain the same storage layout), but one that includes a different method set:
...
With a synced Rinkeby node with RPC exposed on localhost (`Getd --rinkeby --http`) we can make a call against the live <a href="https://rinkeby.etherscan.io/address/0xebe8efa441b9302a0d7eaecc277c09d20d684540" title="Checkpoint Oracle">Checkpoint Oracle</a>, but override its byte code with our own version that has an accessor for the voting threshold field:
